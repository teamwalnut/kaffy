defmodule Api.FixtureSequence do
  @moduledoc """
  Module for generating sequential values.

  Taken from ExMachina:
      https://github.com/thoughtbot/ex_machina/blob/master/lib/ex_machina/sequence.ex

  Use `FixtureSequence.sequence/1` or `FixtureSequence.sequence/2` to generate
  ...
  ## Example
      FixtureSequence.next("joe") # "joe0"
      FixtureSequence.next("joe") # "joe1"
      FixtureSequence.reset()
      FixtureSequence.next("joe") # resets so the return value is "joe0"
  You can use list as well
      FixtureSequence.next("alphabet_sequence", ["A", "B"]) # "A"
      FixtureSequence.next("alphabet_sequence", ["A", "B"]) # "B"
      FixtureSequence.reset()
      FixtureSequence.next("alphabet_sequence"["A", "B"]) # resets so the return value is "A"
  ...
  ## Example
      FixtureSequence.next(:alphabet, ["A", "B", "C"]) # "A"
      FixtureSequence.next(:alphabet, ["A", "B", "C"]) # "B"
      FixtureSequence.reset(:alphabet)
      FixtureSequence.next(:alphabet, ["A", "B", "C"]) # "A"
  And you can also reset multiple sequences at once,
  ## Example
      FixtureSequence.next(:numeric, [1, 2, 3]) # 1
      FixtureSequence.next(:numeric, [1, 2, 3]) # 2
      FixtureSequence.next("joe") # "joe0"
      FixtureSequence.next("joe") # "joe1"
      FixtureSequence.reset(["joe", :numeric])
      FixtureSequence.next(:numeric, [1, 2, 3]) # 1
      FixtureSequence.next("joe") # "joe0"
  """
